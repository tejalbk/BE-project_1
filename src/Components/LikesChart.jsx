    ...
    const fetchPostData = async () => {
      try {
        ...
        const requests = posts.map(post => 
          axios.get(`https://graph.facebook.com/v19.0/${post.id}?fields=like_count&access_token=${accessToken}`)
        );

        const responses = await Promise.all(requests);
        const postData = responses.map(response => response.data.like_count);
